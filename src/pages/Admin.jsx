...
export default function Admin() {
  ...
  const beginEdit = (item) => {
    setEditing(item.id)
    setForm({ q: item.q, options: [...item.options], answer: item.answer })
  }

  const handleAdd = async () => {
    if (!form.q.trim()) return alert('Question text required')
    await addQuestion({ q: form.q, options: form.options, answer: Number(form.answer) })
    await fetchData()
    beginAdd()
  }
  ...
  const handleUpdate = async () => {
    if (!editing) return
    await updateQuestion(editing, { q: form.q, options: form.options, answer: Number(form.answer) })
    await fetchData()
    setEditing(null)
  }
                ...
                {questions.map((item, idx) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-sm text-slate-500 mt-2">
                        {item.options.map((o,i)=> (
                          <div key={i} className={`py-1 ${i===item.answer? 'text-green-600 font-medium':'text-slate-600'}`}>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <button className="btn btn-ghost text-red-600" onClick={() => handleDelete(item.id)}>Delete</button>
                    </div>
                  </motion.div>
                ))}
              </div>
